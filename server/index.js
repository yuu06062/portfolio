...
const db = new sqlite3_1.default.Database("todos.db");
// テーブルがなければ作成
...
app.get("/api/todos", (req, res) => {
    ...
    db.all("SELECT * FROM todos WHERE user_id = ?", [user_id], (err, rows) => {
        ...
        const todos = rows.map(row => (0, index_1.toTodo)(row));
        res.json(todos);
    });
});
// ★ここに追加
...
app.post("/api/todos", (req, res) => {
    ...
    db.run("INSERT INTO todos (text, completed, user_id) VALUES (?, ?, ?)", [text, 0, user_id], function (err) {
        if (err) {
            const errorResponse = { error: err.message };
            res.status(500).json(errorResponse);
            return;
...
app.post("/api/register", (req, res) => {
    ...
    db.run("INSERT INTO users (username, password) VALUES (?, ?)", [username, password], function (err) {
        if (err) {
            const errorResponse = { error: "ユーザー名が既に使われています" };
...
app.post("/api/login", (req, res) => {
    ...
    db.get("SELECT * FROM users WHERE username = ? AND password = ?", [username, password], (err, row) => {
        if (err) {
            const errorResponse = { error: "DBエラー" };
            res.status(500).json(errorResponse);
            return;
        }
        if (!row) {
            const errorResponse = { error: "ユーザー名またはパスワードが違います" };
            res.status(401).json(errorResponse);
            return;
        }
        const loginResponse = (0, index_1.toUser)(row);
